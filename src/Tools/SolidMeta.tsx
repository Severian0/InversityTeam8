  ...
  return {
    addTag(tag: TagDescription) {
      if (cascadingTags.indexOf(tag.tag) !== -1) {
        const properties =
          tag.tag === "title" ? titleTagProperties : metaTagProperties;
  ...
  return {
    addTag(tagDesc: TagDescription) {
      ...
      if (cascadingTags.indexOf(tagDesc.tag) !== -1) {
        const properties =
          tagDesc.tag === "title" ? titleTagProperties : metaTagProperties;
        const tagDescKey = getTagKey(tagDesc, properties);
        const index = tags.findIndex(
          (prev) =>
            prev.tag === tagDesc.tag &&
            getTagKey(prev, properties) === tagDescKey,
        );
        if (index !== -1) {
          tags.splice(index, 1);
        }
...
export const JSONLD: Component<{ children: Record<string, unknown> }> = ({
  children,
}) =>
  MetaTag("script", {
